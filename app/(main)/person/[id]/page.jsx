...
export default function PersonExpensesPage() {
  ...
  return (
    <div className="container mx-auto py-6 max-w-4xl">
      <div className="mb-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
          <div className="flex items-center gap-3">
            <Avatar className="h-16 w-16 ring-2 ring-sky-500/20">
              <AvatarImage src={otherUser?.imageUrl} />
              <AvatarFallback className="bg-gradient-to-br from-sky-500 to-blue-600 text-white font-bold text-xl">
                {otherUser?.name?.charAt(0) || "?"}
              </AvatarFallback>
            </Avatar>
            <div>
              <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-white via-sky-200 to-blue-400 bg-clip-text text-transparent">{otherUser?.name}</h1>
      ...
      {/* Balance card */}
      <Card className="mb-6 border-0 bg-gradient-to-br from-gray-900 to-gray-800 shadow-xl">
        ...
        <CardContent>
          <div className="flex justify-between items-center">
            <div>
              {balance === 0 ? (
                <p className="text-gray-400">You are all settled up</p>
              ) : balance > 0 ? (
                <p className="text-white">
                  <span className="font-medium text-sky-400">{otherUser?.name}</span> owes
                  you
                </p>
              ) : (
                <p className="text-white">
                  You owe <span className="font-medium text-sky-400">{otherUser?.name}</span>
                </p>
              )}
            </div>
            <div
              className={`text-2xl font-bold ${balance > 0 ? "text-green-400" : balance < 0 ? "text-red-400" : "text-white"}`}
            >
