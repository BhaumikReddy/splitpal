    ...
    return (
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="bg-gray-900/50 border border-gray-700 p-4 rounded-lg">
          <h3 className="font-medium mb-2 text-white">Current balance</h3>
          {netBalance === 0 ? (
            <p className="text-gray-300">You are all settled up with {otherUser.name}</p>
          ) : netBalance > 0 ? (
            <div className="flex justify-between items-center">
              <p className="text-gray-300">
                <span className="font-medium text-white">{otherUser.name}</span> owes you
              </p>
            ...
            <div className="flex justify-between items-center">
              <p className="text-gray-300">
                You owe <span className="font-medium text-white">{otherUser.name}</span>
              </p>
        ...
        <div className="space-y-2">
          <Label className="text-white">Who paid?</Label>
          <RadioGroup
          ...
          >
            <div className="flex items-center space-x-2 border border-gray-600 bg-gray-800/50 rounded-md p-3 hover:bg-gray-700/50 transition-colors">
              <RadioGroupItem value="youPaid" id="youPaid" className="border-gray-500 text-sky-400" />
              <Label htmlFor="youPaid" className="flex-grow cursor-pointer text-white">
                <div className="flex items-center">
                  <Avatar className="h-6 w-6 mr-2 ring-1 ring-gray-600">
            ...
            <div className="flex items-center space-x-2 border border-gray-600 bg-gray-800/50 rounded-md p-3 hover:bg-gray-700/50 transition-colors">
              <RadioGroupItem value="theyPaid" id="theyPaid" className="border-gray-500 text-sky-400" />
              <Label htmlFor="theyPaid" className="flex-grow cursor-pointer text-white">
                <div className="flex items-center">
                  <Avatar className="h-6 w-6 mr-2 ring-1 ring-gray-600">
                    <AvatarImage src={otherUser.imageUrl} />
                    <AvatarFallback className="bg-sky-500 text-white">{otherUser.name.charAt(0)}</AvatarFallback>
                  </Avatar>
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <Input
              ...
              step="0.01"
              min="0.01"
              className="pl-7 bg-gray-800 border-gray-600 text-white placeholder:text-gray-400 focus:border-sky-500"
              {...register("amount")}
            />
          </div>
          {errors.amount && (
        ...
        <div className="space-y-2">
          ...
          <Textarea
            ...
            className="bg-gray-800 border-gray-600 text-white placeholder:text-gray-400 focus:border-sky-500 resize-none"
            {...register("note")}
          />
        </div>
        ...
        <Button 
          type="submit" 
          className="w-full bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-600 hover:to-blue-700 text-white rounded-xl shadow-lg hover:shadow-sky-500/25 transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none" 
  ...
  if (entityType === "group") {
    ...
    return (
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        {/* Select group member */}
        <div className="space-y-2">
          <Label className="text-white">Who are you settling with?</Label>
          <div className="space-y-2">
            {groupMembers.map((member) => {
              ...
              return (
                <div
                  key={member.userId}
                  className={`border border-gray-600 bg-gray-800/50 rounded-md p-3 cursor-pointer transition-colors ${
                    isSelected
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Avatar className="h-8 w-8 ring-1 ring-gray-600">
                        ...
                        <AvatarFallback className="bg-sky-500 text-white">{member.name.charAt(0)}</AvatarFallback>
                      </Avatar>
